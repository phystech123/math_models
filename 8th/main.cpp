...
void eyler(double* arr, int N, double* X, double* V){
    for(int i = 1; i < N; i++){
        ...
        X[i] = X[i-1] + h*V[i-1];
        V[i] = V[i-1] - h*(pow(W, 2) * X[i-1] + 2*g*V[i-1]);
    }
}
...
void hoin(double* arr, int N, double* X, double* V){
        ...
        // V(i+1) = v(i) - w**2*x
            double h = arr[i] - arr[i-1];
            X[i] = X[i-1] + h*(V[i-1] + V[i-1] - h*(pow(W, 2) * X[i-1] + 2*g*V[i-1]))/2;
            V[i] = V[i-1] - h*(pow(W, 2) * ((X[i-1] + X[i-1] + h*V[i-1])/2) +2*g*(V[i-1] + V[i-1] - h*(pow(W, 2) * X[i-1] + 2*g*V[i-1]))/2 ); 
        } 
}
...
// ---------------------------------------------------------------
        ...
        double k1_x = h*V[i-1];
        double k1_v = h*(-pow(W, 2)*(X[i-1]) - 2*g*V[i-1]);

        double k2_x = h*(V[i-1] + k1_v/2.);
        double k2_v = h*(-pow(W, 2)*(X[i-1] + k1_x/2.) - 2*g*(V[i-1] + k1_v/2.));

        double k3_x = h*(V[i-1] + k2_v/2.);
        double k3_v = h*(-pow(W, 2)*(X[i-1] + k2_x/2.) - 2*g*(V[i-1] + k2_v/2.));

        double k4_x = h*(V[i-1] + k3_v);
        double k4_v = h*(-pow(W, 2)*(X[i-1] + k3_x)  - 2*g*(V[i-1] + k3_v));
